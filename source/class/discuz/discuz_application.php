...
class discuz_application extends discuz_base{
	...
	/*instance()原来是按址引用的，并且是实理化自身了，这个类继承自抽象类discuz_base，这个应该是个基类，来看看构造函数。*/
	...
	public function __construct() {
		$this->_init_env();
		// 274-316行；初始化环境变量。定义了些常量，引入了公共函数库，在./source/function/function_core.php,设置最小内存128M。并设置了允许的全局变量(_GET,_POST,_REQUEST,_COOKIE,_SERVER,_ENV,_FILES),定义了一个全局变量$_G，变将这个全局变量按址传递给了$this->var,看来在模板中可以直接使用这些变量的。

		$this->_init_config();
		// 274-316行；初始化配置变量。引入了配置,在./config/config_global.php，将配置变量全部压入到$this->var['config']中，在全局都可以使用配置啦。

		$this->_init_input();
		// 224-272行；初始化输入。过滤了下全局变量（$_GET['GLOBALS'],$_POST['GLOBALS']……）,如果开启了get_magic_quotes_gpc()，就给$_GET,$_POST,$_COOKIE变量stripslashes一下，设置合法的cookie的键名必须 $this->config['cookie']['cookiepre']这个开头的，在配置文件中有。这样做应该是防止非法cookie变量，243行把$_POST的值合并到$_GET变量中去。把rawurlencode($_GET['page'])编码下，过滤了下$_GET['handlekey']，过滤$_GET变量，foreach($_GET as $k => $v) {$this->var['gp_'.$k] = daddslashes($v);}使用的是stripslashes函数。另外设置了几个全局变量 $this->var['XXX']自己瞅瞅。

		$this->_init_output();
		// 318-345行；初始化输出。过滤URL,$this->_xss_check();<"CONTENT-TRANSFER-ENCODING都是非法的。如果控制器为'seccode', 'secqaa', 'swfupload'则进行这个过滤，有时间看看./source/include/misc/misc_security.php;开启ob_start()，设置charset；
	}
	...
	public function reject_robot() {
 	...
 	/*
 	 * 字符验证 被注释掉的会在提交富文本时报错
 	 * 提示信息:您当前的访问请求当中含有非法字符,已经被系统拒绝
 	*/
