...
case 'joinpage':
	...
	$formcheck['url_return_ok'] = AURL.'&pluginop=joinlist';
	$formcheck['url_return_no'] = AURL.'&pluginop=joinpage';
...
case 'joinpage_update':
	...
	if($str){
		cpmsg('修改成功',AURLJUMP.'&pluginop=joinlist','succeed');
	}else{
		cpmsg('修改失败','','error');
	}
	break;
...
case 'giftpage':
	...
	$formcheck['url_return_ok'] = AURL.'&pluginop=gift';
	$formcheck['url_return_no'] = AURL.'&pluginop=giftpage';
	$loid = $_GET['loid']?intval($_GET['loid']):0;
	$oppo = looppo('gift');
	$table = $oppo['table'];
	$lokey = $oppo['lokey'];
