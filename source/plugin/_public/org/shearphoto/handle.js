...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "../shearphoto_common"; //修改路径在这边，你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	...
	Shear.config({
		...
		proportional: [0.75, <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			/*
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			completeA.onclick || (completeA.onclick = function() {
				...
				this_.again();
				this_.pointhandle(3e3, 10, "上传完成！已返回！", 2, "#fbeb61", "#3a414c");
			});
		}
		/*--------------------------------------------------------------截图成功后，返回来的callback-------------------------*/
	...
	Shear.addEvent(document.getElementById("saveShear"), "click", function() { //按下截图事件，提交到后端的shearphoto.php接收
		var result=document.getElementById('zp_info').value;
		if(result==""){
			$(this).parent().parent().find('.zp_info1').css('display','none');
			$(this).parent().parent().find('.zp_info').css('display','inline-block');
			return false;
		}else if(result.length<5){
			$(this).parent().parent().find('.zp_info1').css('display','none');
			$(this).parent().parent().find('.zp_info').css('display','none');
			$(this).parent().parent().find('.zp_info2').css('display','block');
			return false;
		}
		Shear.SendPHP({
			shearphoto: "我要传参数到服端",
