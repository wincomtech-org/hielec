...
if($operation == 'patch' || $operation == 'cross') {
	...
	if($step == 1) {
		...
		showtablerow('', '', array($lang['founder_upgrade_store_directory'].'./data/update/ X'.$version.' Release['.$release.']'));
		...
		showtablerow('', 'colspan="2"', $lang['founder_upgrade_download_file'].' ./data/update/ X'.$version.' Release['.$release.']'.'');
		showtablerow('', 'colspan="2"', $lang['founder_upgrade_backup_file'].' ./data/back/ '.DISCUZ_VERSION.' Release['.DISCUZ_RELEASE.']'.$lang['founder_upgrade_backup_file2']);
		...
		if(!$_GET['startupgrade']) {
			...
			foreach($updatefilelist as $updatefile) {
				...
				$backfile = DISCUZ_ROOT.'./data/back/ X'.substr(DISCUZ_VERSION, 1).' Release['.DISCUZ_RELEASE.']/'.$updatefile;
				if(is_file($destfile)) {
		...
		foreach($updatefilelist as $updatefile) {
			$srcfile = DISCUZ_ROOT.'./data/update/ X'.$version.' Release['.$release.']/'.$updatefile;
			if($confirm == 'ftp') {
				$destfile = $updatefile;
			} else {
				$destfile = DISCUZ_ROOT.$updatefile;
		...
		if($upgradeinfo['isupdatedb']) {
			...
			foreach($dbupdatefilearr as $dbupdatefile) {
				$srcfile = DISCUZ_ROOT.'./data/update/ X'.$version.' Release['.$release.']/'.$dbupdatefile;
		...
		$file = DISCUZ_ROOT.'./data/update/ X'.$version.' Release['.$release.']/updatelist.tmp';
		@unlink($file);
	...
	$upgrade_step = C::t('common_cache')->fetch('upgrade_step');
	$upgrade_step = dunserialize($upgrade_step['cachevalue']);
	$file = DISCUZ_ROOT.'./data/update/ X'.$upgrade_step['version'].' Release['.$upgrade_step['release'].']/updatelist.tmp';
	@unlink($file);
	@unlink(DISCUZ_ROOT.'./install/update.php');
	C::t('common_cache')->delete('upgrade_step');
	C::t('common_cache')->delete('upgrade_run');
