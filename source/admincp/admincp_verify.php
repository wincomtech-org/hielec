...
if($operation == 'verify') {
	...
	if($anchor == 'add') {
		...
		if(!submitcheck('verifysubmit', true)) {
			...
			$datehtml = $orderbyhtml = '';// 提交时间html 和 结果排序html
			if($anchor != 'pass') {
...
// $vid = 6;
			...
			$results = getwheres($intkeys, $strkeys, $randkeys, $likekeys, 'v.');// 用户验证信息
			// 循环 $likekeys 这里只有 username
			...
			echo "<script>disallowfloat = '{$_G[setting][disallowfloat]}';</script><input type=\"hidden\" name=\"verifysubmit\" value=\"true\" />";
			...
			if($anchor != 'pass') {
				...
				// \source\class\table\table_common_member_verify_info.php
