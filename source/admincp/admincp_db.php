	...
	if(!submitcheck('exportsubmit')) {
		...
		if($_GET['method'] == 'multivol') {
			...
			if(trim($sqldump)) {
				$sqldump = "$idstring".
					"# <?php exit();?>\n".
					"# Wincomtech! Multi-Volume Data Dump Vol.$volume\n".
					"# Version: Wincomtech! {$_G[setting][version]}\n".
					"# Time: $time\n".
					"# Type: {$_GET['type']}\n".
					"# Table Prefix: $tablepre\n".
					"#\n".
					"# Wincomtech! Home: http://www.wincomtech.cn\n".
					"# Please visit our website for newest infomation about \n".
					"# --------------------------------------------------------\n\n\n".
					"$setnames".
					$sqldump;
