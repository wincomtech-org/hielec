...
if($operation == 'export' && $id) {
	...
	if($isplugindeveloper || !$addonid || !cloudaddons_getmd5($addonid)) {
		...
		exportdata(' Style', $stylearray['name'], $stylearray);
	} else {
		cpheader();
		cpmsg('styles_export_invalid', '', 'error');
	}
	...
	if(!submitcheck('stylesubmit')) {
		...
		foreach($sarray as $id => $style) {
			...
			$stylelist .=
				...
				"
				//注释界面->风格管理->默认模板套系->手机版-开始
				// .($d1exists ? "
				// 	<p style=\"margin: 1px 0\"><label><input type=\"radio\" class=\"radio\" name=\"defaultnew1\" value=\"$id\" $isdefault1 /> $lang[styles_default1]</label></p>" : "<p style=\"margin: 1px 0\" class=\"lightfont\"><label><input type=\"radio\" class=\"radio\" disabled readonly /> $lang[styles_default1]</label></p>")."
