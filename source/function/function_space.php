...
function &getlayout($layout='',$width=1200) {
	$layoutarr = array(
	...
	if (!empty($layout)) {
		if ($width) {
			$widthper = explode(':', $layout);
			$num = array_sum($widthper);
			foreach ($widthper as $v) {
				if ($v) {
					$rt[] = ($v/$num)*$width;
				}
			}
		...
		foreach ($layoutarr as $key => $value) {
			$widthper = explode(':', $key);
			$num = array_sum($widthper);
			foreach ($widthper as $k => $v) {
				if ($v) {
					$layoutarr_temp[$key][$k] = ($v/$num)*$width;
				}
			}
		}
		$rt = $layoutarr = $layoutarr_temp;
	}
