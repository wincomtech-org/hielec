...
$addon = $addonsource ?
	...
	array(
		'website_url' => 'http://admin.php?action=plugins',
		'download_url' => 'http://admin.php?action=plugins/index.php',
		'download_ip' => '',
...
function cloudaddons_getuniqueid() {
	global $_G;
	if(CLOUDADDONS_WEBSITE_URL == 'http://admin.php?action=plugins') {
		...
		while($f = $d->read()) {
			if(preg_match('/^discuz\_'.$dirs[$type][0].'\_'.$id.'(\_\w+)?\.xml$/', $f)) {
				...
				if($type == 'plugin' && !$filedeleted) {
					...
					$pluginarray = getimportdata(' Plugin');
