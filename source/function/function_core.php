...
function formhash($specialadd = '') {
	global $_G;
	$hashadd = defined('IN_ADMINCP') ? 'Only For Wincomtech! Admin Control Panel' : '';
...
function dnumber($number,$dot='') {
	// sprintf("%.2f", $number);
	// number_format($number, 2, '.', '');
	// round($number,2);
	if ($dot) {
		$number = number_format($number,$dot,'.','');
	}
...
function withdraw_config($allow='switch,min,max,ratio,day,type,blacklist',$wdlang='',$mkey='withdraw')
{
    $allow = explode(',', $allow);
    foreach ($allow as $k => $v) {
        // $arrAllow[$mkey .'_'. $v]['lang'] = $wdlang[$k];
        // if ($strAllow) { $strAllow .= ','; } $strAllow .= '\''. $mkey .'_'. $v .'\'';
        $strAllow .= $strAllow ? ',\''. $mkey .'_'. $v .'\'' : '\''. $mkey .'_'. $v .'\'';
    }
    $sql = sprintf('SELECT * from %s where skey in (%s)',DB::table('common_setting'),$strAllow);
    $getextension = C::memory()->getextension();
    if ($getextension['memcache']) {
        $mkey = strrev($mkey.'_config');
        if (strlen($mkey)>250) { $mkey = substr($mkey,0,250); }
        if (memory('get',$mkey)===false) {
            $withdraw_config = DB::fetch_all($sql);
            foreach ($withdraw_config as $value) {
                $wd_conf[0][$value['skey']] = $value['skey'];
                $wd_conf[1][$value['skey']] = ($value['svalue']) ? $value['svalue'] : '';
            }
            memory('set', $mkey, $wd_conf);// 一直有效
        } else {
            $wd_conf = memory('get',$mkey);
            if (empty($wd_conf)) memory('rm',$mkey);
        }
        // memory('rm',$mkey);
        // C::memory()->clear();
        // C::memory()->close();
