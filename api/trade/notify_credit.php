...
if($notifydata['validator']) {
	...
	if($order && floatval($postprice) == floatval($order['price']) && ($apitype == 'tenpay' || strtolower($_G['setting']['ec_account']) == strtolower($_REQUEST['seller_email']))) {
		...
		if($order['status'] == 1) {
			C::t('forum_order')->update($orderid, array('status' => '2', 'buyer' => "$notifydata[trade_no]\t$apitype", 'confirmdate' => $_G['timestamp']));// 更新订单状态为已支付
			updatemembercount($order['uid'], array($_G['setting']['creditstrans'] => $order['amount']), 1, 'AFD', $order['uid']);// 更新用户积分
			updatecreditbyaction($action, $uid = 0, $extrasql = array(), $needle = '', $coef = 1, $update = 1, $fid = 0);// 积分日志？
			C::t('forum_order')->delete_by_submitdate($_G['timestamp']-60*86400);// 删除超过两个月的充值记录
